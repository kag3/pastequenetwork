...
public class WorldWiper {
    ...
    public void wipeOnFirstRun() {
        ...
        if (existing != null) {
            ...
            if (!unloaded) {
                plugin.getLogger().warning("Impossible de decharger 'world'. "
                        + "Le wipe sera saute : la schematic sera pastee sur le terrain existant.");
                pasteLobbySchematicFallback(existing);
                try { marker.createNewFile(); } catch (IOException ignored) {}
                return;
            }
        }
        ...
        // Marque aussi .lobby-loaded pour que LobbyManager.setup() ne reposte pas
    ...
    private void pasteLobbySchematicFallback(World world) {
        File schem = findLobbySchematic();
        ...
        Location spawn = new Location(world,
                ...
                cfg.getDouble("lobby.spawn.z", 0.5));
        Location paste = spawn.clone().subtract(0, 10, 0);
        new SchematicLoader(plugin).paste(schem, paste);
    }

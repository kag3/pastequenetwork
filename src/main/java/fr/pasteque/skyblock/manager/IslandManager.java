...
public class IslandManager {
    ...
    public void load() {
        ...
        for (String ownerKey : section.getKeys(false)) {
            ...
            island.setPvpEnabled(islandSection.getBoolean("pvpEnabled", plugin.getConfig().getBoolean("pvp.enabled-by-default", false)));
    ...
    public int getMaxMembers() { return plugin.getConfig().getInt("island.max-members", 6); }
    ...
    public Island createIslandFor(UUID owner, String ownerName) {
        ...
        int gap = plugin.getConfig().getInt("island.gap", 340);
        ...
        island.setPublicVisit(plugin.getConfig().getBoolean("island.default-public", false));
        island.setPvpEnabled(plugin.getConfig().getBoolean("pvp.enabled-by-default", false));
        island.setBankBalance(plugin.getConfig().getDouble("island.starter-bank", 0.0D));
        island.setName(defaultIslandName(ownerName));
        island.setInviteUnlocks(0);
        island.setHome(new Location(world, centerX + 0.5D, plugin.getConfig().getInt("island-world-y", 100) + 2, centerZ + 0.5D));
        ownedIslands.put(owner, island);
    ...
    private void setCircle(World world, int cx, int y, int cz, int radius, Material top, Material fill) {
        for (int x = cx - radius; x <= cx + radius; x++) for (int z = cz - radius; z <= cz + radius; z++) {
            int dx = x - cx; int dz = z - cz;
            if ((dx * dx) + (dz * dz) <= radius * radius) fillColumn(world, x, y, z, top, fill);
        }
    }

    private void setRect(World world, int minX, int maxX, int y, int minZ, int maxZ, Material top, Material fill) {
        for (int x = minX; x <= maxX; x++) {
            for (int z = minZ; z <= maxZ; z++) {
                fillColumn(world, x, y, z, top, fill);
            }
        }
    }
    ...
    private void terrainColumn(World world, int x, int baseY, int z, int height, Material top, Material fill) {
        ...
        for (int i = 1; i <= 4; i++) {
            world.getBlockAt(x, topY - i, z).setType(fill);
        }
    ...
    public void generateStarterIsland(Island island) {
        World world = plugin.getWorldManager().getOrCreateIslandWorld();
        int y = plugin.getConfig().getInt("island-world-y", 100);
        ...
        if (plugin.getConfig().getBoolean("island.starter.chest", true)) {
            chest.getBlockInventory().addItem(new ItemStack(Material.ICE, plugin.getConfig().getInt("island.starter.ice", 1)));
            if (plugin.getConfig().getBoolean("island.starter.water-bucket", true)) chest.getBlockInventory().addItem(new ItemStack(Material.WATER_BUCKET, 1));
            if (plugin.getConfig().getBoolean("island.starter.lava-bucket", true)) chest.getBlockInventory().addItem(new ItemStack(Material.LAVA_BUCKET, 1));
            chest.getBlockInventory().addItem(new ItemStack(Material.MELON, plugin.getConfig().getInt("island.starter.melon-slices", 4)));
        ...
        int treeOneY = getTopY(world, treeOneX, treeOneZ, y, y + 14) + 1;
        int treeTwoY = getTopY(world, treeTwoX, treeTwoZ, y, y + 14) + 1;
        placeTree(world, treeOneX, treeOneY, treeOneZ);
        placeTree(world, treeTwoX, treeTwoY, treeTwoZ);
    ...
    private void generateFarmingIsland(Island island) {
        ...
        for (int i = 0; i < 4; i++) {
            ...
            int top = getTopY(world, tx, tz, y, y + 18) + 1;
            placeTree(world, tx, top, tz);
        }
    ...
    private int getTopY(World world, int x, int z, int minY, int maxY) {
        for (int yy = maxY; yy >= minY; yy--) {
            if (world.getBlockAt(x, yy, z).getType() != Material.AIR) return yy;
        }
        return minY;
    }
            ...
            for (int z = targetZ - 17; z <= targetZ + 17; z++) {
                ...
                for (int depth = 1; depth <= depthMax; depth++) {
                    if (random.nextInt(100) < 16) {
                        world.getBlockAt(x, topY - depth, z).setType(pickOre(random));
                    }
                }
            }
        }
    ...
    private IslandRollback createRollback(Island island, boolean deletedIsland) {
        ...
        int size = plugin.getConfig().getInt("island.size", 320);
        int half = size / 2;
        int minY = plugin.getConfig().getInt("island.protection-height-min", 0);
        int maxY = plugin.getConfig().getInt("island.protection-height-max", 255);
        List<BlockSnapshot> blocks = new ArrayList<BlockSnapshot>();
        for (int x = island.getCenterX() - half; x <= island.getCenterX() + half; x++) for (int z = island.getCenterZ() - half; z <= island.getCenterZ() + half; z++) for (int y = minY; y <= maxY; y++) {
            Block block = world.getBlockAt(x, y, z);
            if (block.getType() != Material.AIR || block.getState() instanceof Chest) blocks.add(BlockSnapshot.capture(block));
        }
    ...
    private void clearIslandArea(Island island) {
        ...
        int minY = plugin.getConfig().getInt("island.protection-height-min", 0);
        int maxY = plugin.getConfig().getInt("island.protection-height-max", 255);
        for (int x = island.getCenterX() - half; x <= island.getCenterX() + half; x++) for (int z = island.getCenterZ() - half; z <= island.getCenterZ() + half; z++) for (int y = minY; y <= maxY; y++) world.getBlockAt(x, y, z).setType(Material.AIR);
    }
    ...
    public boolean invite(Player owner, Player target) {
        ...
        long expireAt = System.currentTimeMillis() + (plugin.getConfig().getLong("confirm.expire-seconds", 20L) * 1000L);
    ...
    public int getLevel(Island island) {
        ...
        int total = 0;
        for (int x = island.getCenterX() - half; x <= island.getCenterX() + half; x++) for (int z = island.getCenterZ() - half; z <= island.getCenterZ() + half; z++) for (int y = minY; y <= maxY; y++) total += values.getInt(world.getBlockAt(x, y, z).getType().name(), 0);
        return total;
    }
    ...
    public Location getSafeTeleport(Island island) {
        ...
        if (island != null) return new Location(plugin.getWorldManager().getOrCreateIslandWorld(), island.getCenterX() + 0.5D, plugin.getConfig().getInt("island-world-y", 100) + 2, island.getCenterZ() + 0.5D);

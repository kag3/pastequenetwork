...
public class FilterService {
    ...
    public FilterService(PastequeGuardPlugin plugin, SanctionService sanctionService) {
        this.plugin = plugin;
    ...
    public boolean shouldBlock(Player player, String message) {
        ...
        player.sendMessage(plugin.getPrefix() + plugin.color(plugin.getConfig().getString("messages.anti-spam-warn", "&fMerci d'arrêter de spam/flood sous peine de sanction.")));
        if (total >= 4) {
            long duration = fr.pasteque.guard.util.TimeUtil.parseDuration(plugin.getConfig().getString("auto-moderation.automute-duration", "10m"));
            ...
            player.sendMessage(plugin.getPrefix() + plugin.color(plugin.getConfig().getString("messages.auto-muted", "&fVous avez été temporairement réduit au silence pour spam/flood répété.")));

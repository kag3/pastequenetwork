...
public class SanctionService {
    ...
    public SanctionService(PastequeGuardPlugin plugin) {
        this.plugin = plugin;
    ...
    public synchronized void applySanction(String targetName, UUID targetUuid, String staffName, SanctionType type, String reason, long durationMs) {
        ...
        if (targetUuid != null) {
            config.set(base + ".uuid", targetUuid.toString());
        }
    ...
    public String buildMuteMessage(SanctionEntry sanction) {
        return plugin.getPrefix() + plugin.color(plugin.getConfig().getString("mute-message", "&fVous ne pouvez pas parler actuellement. &fRaison : &d%reason% &f| Fin : &d%expires%")

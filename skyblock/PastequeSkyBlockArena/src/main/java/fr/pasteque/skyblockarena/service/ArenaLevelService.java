...
public class ArenaLevelService {
    ...
    public ArenaLevelService(PastequeSkyBlockArenaPlugin plugin, PlayerDataService playerDataService) {
        this.plugin = plugin;
    ...
    private void levelUpIfNeeded(Player player, ArenaPlayerData data) {
        ...
        while (data.getLevel() < maxLevel) {
            ...
            player.sendTitle(plugin.color(plugin.getConfig().getString("messages.rankup-title", "&d[ %from% ] &f➜ &5[ %to% ]").replace("%from%", String.valueOf(from)).replace("%to%", String.valueOf(to))),
                    plugin.color(plugin.getConfig().getString("messages.rankup-subtitle", "&fVotre expérience d'arène progresse.")));
            player.playSound(player.getLocation(), Sound.ENTITY_PLAYER_LEVELUP, 1.0F, 1.15F);
            player.sendMessage(plugin.color(plugin.prefix() + plugin.getConfig().getString("messages.level-up-chat", "&fVous passez au niveau &d%to%&f. Continuez comme ça !").replace("%to%", String.valueOf(to))));
        }
    }
}

...
public class SocialManager {
    ...
    public void save() {
        dataFile.getConfig().set("friends", null); dataFile.getConfig().set("enemies", null); dataFile.getConfig().set("alliances", null);
    ...
    public boolean inviteAlliance(UUID target, String name) { if (!allianceMembers.containsKey(name.toLowerCase()) || alliancesByPlayer.containsKey(target)) return false; pendingAlliances.put(target, name.toLowerCase()); return true; }
    ...
    public void denyAlliance(UUID target) { pendingAlliances.remove(target); }

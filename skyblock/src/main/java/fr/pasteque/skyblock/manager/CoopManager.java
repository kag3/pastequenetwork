...
public class CoopManager {
            ...
            @Override public void run() {
                ...
                while (it.hasNext()) {
                    ...
                    if (pending.expireAt < System.currentTimeMillis()) {
                        notifyCancel(pending, "&fLa création de l'île coop a expiré.");
    ...
    public boolean beginCreation(Player owner, List<Player> invitedPlayers) {
        if (pendingByOwner.containsKey(owner.getUniqueId())) {
            MessageUtil.send(owner, plugin.getPrefix(), "&fUne création de coop est déjà en attente.");
            return false;
        }
        ...
        for (Player player : invitedPlayers) {
            ...
            if (inviteTargetToOwner.containsKey(player.getUniqueId())) {
                MessageUtil.send(owner, plugin.getPrefix(), "&f" + player.getName() + " a déjà une invitation coop en attente.");
                return false;
            }
            invitees.add(player.getUniqueId());
        }
        if (invitees.isEmpty()) {
            MessageUtil.send(owner, plugin.getPrefix(), "&fTu dois inviter au moins 1 joueur connecté.");
            return false;
        }
        if (invitees.size() > 5) {
            MessageUtil.send(owner, plugin.getPrefix(), "&fMaximum 5 joueurs invités (6 avec toi).");
            return false;
        }
    ...
    public boolean acceptInvite(Player player) {
        ...
        if (owner != null) {
            MessageUtil.send(owner, plugin.getPrefix(), "&a" + player.getName() + " &fa accepté l'invitation coop. Reste &e" + pending.remaining() + " &fvalidation(s).");
            sendProgress(owner, pending);
        }
    ...
    public boolean denyInvite(Player player) {
        ...
        notifyCancel(pending, "&fLa création de la Coop Island a été annulée : &d" + player.getName() + " &fa refusé.");
    ...
    private void finalizeCreation(PendingCreation pending) {
        ...
        for (UUID uuid : island.getMembers()) {
            ...
            if (member != null && member.isOnline()) {
                ...
                MessageUtil.send(member, plugin.getPrefix(), "&aLa Coop Island est prête ! Bienvenue sur &f" + island.getName());
            }
        }
    ...
    public CoopIsland createIsland(UUID owner, List<UUID> additionalMembers) {
        ...
        int gap = plugin.getConfig().getInt("coop.gap", 520);
        int index = nextIndex++;
        int gx = index % 4000;
        int gz = index / 4000;
        int centerX = gx * gap + gap;
        int centerZ = gz * gap + gap;
        int size = plugin.getConfig().getInt("coop.base-size", 220) + ((total - 2) * plugin.getConfig().getInt("coop.per-player-extra-size", 30));
        ...
        island.setHome(new Location(world, centerX + 0.5D, plugin.getConfig().getInt("coop-world-y", 100) + 4, centerZ + 0.5D));
        islands.put(id, island);
    ...
    public void generateStarterIsland(CoopIsland island, int players) {
        World world = plugin.getWorldManager().getOrCreateCoopWorld();
        int y = plugin.getConfig().getInt("coop-world-y", 100);
    ...
    public Location getSafeTeleport(CoopIsland island) {
        ...
        return new Location(plugin.getWorldManager().getOrCreateCoopWorld(), island.getCenterX() + 0.5D, plugin.getConfig().getInt("coop-world-y", 100) + 2, island.getCenterZ() + 0.5D);
    }

...
public class EndEventManager {
    ...
    public void start() {
        Bukkit.getScheduler().runTaskTimer(plugin, new Runnable() {
            @Override public void run() {
                ...
                if (!active && now.get(Calendar.HOUR_OF_DAY) == plugin.getConfig().getInt("end-event.hour", 20) && now.get(Calendar.MINUTE) == 0 && lastDailyLaunchDay != day) {
                    lastDailyLaunchDay = day;
                    launch(false);
                }
                if (active) {
                    long remaining = endAt - System.currentTimeMillis();
                    if (remaining <= 0L) { stop(false); return; }
    ...
    public void stop(boolean dragonKilled) {
        if (!active) return;
        World world = plugin.getWorldManager().getEndEventWorld();
        if (dragonKilled) announceWinners();
        else broadcast("&5Fin de l'End Event &f: personne n'a reussi a tuer le &dPasteque Dragon&f a temps.");
    ...
    private void announceWinners() {
        ...
        Collections.sort(list, new Comparator<Map.Entry<UUID, Double>>() {
            @Override public int compare(Map.Entry<UUID, Double> a, Map.Entry<UUID, Double> b) { return Double.compare(b.getValue(), a.getValue()); }
        });
        double[] rewards = new double[]{
            plugin.getConfig().getDouble("end-event.rewards.first", 100000.0D),
            plugin.getConfig().getDouble("end-event.rewards.second", 50000.0D),
            plugin.getConfig().getDouble("end-event.rewards.third", 25000.0D),
            plugin.getConfig().getDouble("end-event.rewards.fourth", 25000.0D)
        };
        String[] labels = new String[]{"1er", "2e", "3e", "4e"};
        broadcast("&aLe &dPasteque Dragon &aa ete vaincu ! Recompenses des meilleurs degats :");
        ...
        double lastHitReward = plugin.getConfig().getDouble("end-event.rewards.last-hit", 15000.0D);
        if (lastHit != null) {
            plugin.getEconomyManager().add(lastHit, lastHitReward);
            String name = Bukkit.getOfflinePlayer(lastHit).getName();
            broadcast("&dDernier coup fatal &f: &e" + (name == null ? lastHit.toString() : name) + " &f+ &a" + ((int) lastHitReward) + " Pasteque");
        }
    ...
    public void recordDamage(Player player, Entity entity, double amount) {
        ...
        UUID hitDragon = null;
        if (entity instanceof EnderDragon) hitDragon = entity.getUniqueId();
        else if (entity instanceof ComplexEntityPart) {
            Entity parent = ((ComplexEntityPart) entity).getParent();
            if (parent instanceof EnderDragon) hitDragon = parent.getUniqueId();
        }
    ...
    private EnderDragon getTrackedDragon(World world) {
        ...
        if (dragonId != null) {
            for (Entity entity : world.getEntities()) if (entity.getUniqueId().equals(dragonId) && entity instanceof EnderDragon) return (EnderDragon) entity;
        }
        for (Entity entity : world.getEntities()) if (entity instanceof EnderDragon) { dragonId = entity.getUniqueId(); return (EnderDragon) entity; }
        return null;
    }
    ...
    public void handleDragonDeath(Player killer, Entity entity) {
        ...
        if (System.currentTimeMillis() < stabilizationUntil) {
            World world = plugin.getWorldManager().getEndEventWorld();
            if (world != null) Bukkit.getScheduler().runTaskLater(plugin, new Runnable() {
    ...
    private Location getDragonSpawn(World world) {
        ...
        if (base == null || base.getWorld() == null) base = new Location(world, 0.5D, 90.0D, 0.5D);
        if (!world.getName().equalsIgnoreCase(base.getWorld().getName())) base = new Location(world, base.getX(), base.getY(), base.getZ(), base.getYaw(), base.getPitch());
    ...
    public void setEventSpawn(Location location) { plugin.getPvpManager().setNamedWarp("endevent-spawn", location); }

    public void teleport(Player player) {
        if (!active) { MessageUtil.send(player, plugin.getPrefix(), "&fAucun End Event n'est actif actuellement."); return; }
        player.teleport(getEventSpawn());
        MessageUtil.send(player, plugin.getPrefix(), "&aTeleportation vers l'End Event.");
    }

    private void broadcast(String msg) { Bukkit.broadcastMessage(MessageUtil.color(plugin.getPrefix() + msg)); }
}

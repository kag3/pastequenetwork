...
public class ChallengeManager {
    ...
    public void load() {
        ...
        for (String challengeId : section.getKeys(false)) {
            ...
            for (String value : values) uuids.add(UUID.fromString(value));
    ...
    public Set<String> getChallengeIds() {
        ConfigurationSection section = plugin.getConfig().getConfigurationSection("challenges.list");
    ...
    public boolean complete(Player player, String challengeId) {
        if (isCompleted(challengeId, player.getUniqueId())) return false;
        ConfigurationSection section = plugin.getConfig().getConfigurationSection("challenges.list." + challengeId);
        if (section == null) return false;
        Material material = Material.matchMaterial(section.getString("material", "STONE"));
        int amount = section.getInt("amount", 1);
        if (material == null || !hasItems(player, material, amount)) return false;
        ...
        if (!completed.containsKey(challengeId)) completed.put(challengeId, new HashSet<UUID>());

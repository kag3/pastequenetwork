...
public class GuardCommand implements CommandExecutor, TabCompleter {
    ...
    public GuardCommand(PastequeSkyblockPlugin plugin, SanctionService sanctionService, ReportService reportService, FilterService filterService) {
        this.plugin = plugin;
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (!sender.hasPermission("pastequeguard.staff")) {
            sender.sendMessage(plugin.getGuardPrefix() + plugin.color(plugin.getConfig().getString("messages.no-permission")));
            return true;
        }

        if (sub.equals("panel")) {
            if (!(sender instanceof Player)) {
                sender.sendMessage(plugin.getGuardPrefix() + plugin.color(plugin.getConfig().getString("messages.player-only")));
                return true;
            }
    ...
    private boolean handleReport(CommandSender sender, String[] args) {
        ...
        if (args.length < 3) {
            sender.sendMessage(plugin.getGuardPrefix() + plugin.color(plugin.getConfig().getString("messages.syntax").replace("%syntax%", "/pg report <joueur> <motif>")));
            return true;
        }
        Player player = (Player) sender;
        String target = args[1];
        if (target.equalsIgnoreCase(player.getName())) {
            player.sendMessage(plugin.getGuardPrefix() + plugin.color("&dVous ne pouvez pas vous signaler vous-même."));
            return true;
        }
        String reason = join(args, 2);
        reportService.createReport(player.getName(), target, reason);
        player.sendMessage(plugin.getGuardPrefix() + plugin.color(plugin.getConfig().getString("messages.report-received")
                .replace("%target%", target)));
        String notify = plugin.getGuardPrefix() + plugin.color(plugin.getConfig().getString("messages.report-notify")
    ...
    private boolean handleSanction(CommandSender sender, String sub, String[] args) {
        if (sub.equals("kick") || sub.equals("warn")) {
            if (args.length < 3) {
                sender.sendMessage(plugin.getGuardPrefix() + plugin.color(plugin.getConfig().getString("messages.syntax").replace("%syntax%", "/pg " + sub + " <joueur> <raison>")));
                return true;
            }
        } else if (sub.equals("tempban") || sub.equals("tempmute")) {
            if (args.length < 4) {
                sender.sendMessage(plugin.getGuardPrefix() + plugin.color(plugin.getConfig().getString("messages.syntax").replace("%syntax%", "/pg " + sub + " <joueur> <durée> <raison>")));
                return true;
            }
        } else {
            if (args.length < 3) {
        ...
        if (sub.equals("kick")) {
            ...
            sender.sendMessage(plugin.getGuardPrefix() + plugin.color(plugin.getConfig().getString("messages.sanction-applied").replace("%target%", targetName)));
            return true;
        }
        ...
        if (sub.equals("warn")) {
            ...
            if (online != null) {
                online.sendMessage(plugin.getGuardPrefix() + plugin.color(plugin.getConfig().getString("messages.warn-received").replace("%reason%", reason)));
            }
            sender.sendMessage(plugin.getGuardPrefix() + plugin.color(plugin.getConfig().getString("messages.warn-sent").replace("%target%", targetName)));
            return true;
        }
        ...
        if (sub.equals("tempban")) {
            ...
            if (duration <= 0L) {
                sender.sendMessage(plugin.getGuardPrefix() + plugin.color("&dDurée invalide. Exemple : 30m, 2h, 7d"));
                return true;
            }
    ...
    private boolean handleUnban(CommandSender sender, String[] args) {
        if (args.length < 2) {
            sender.sendMessage(plugin.getGuardPrefix() + plugin.color(plugin.getConfig().getString("messages.syntax").replace("%syntax%", "/pg unban <joueur>")));
            return true;
        }
        sanctionService.unban(args[1], sender.getName());
        sender.sendMessage(plugin.getGuardPrefix() + plugin.color(plugin.getConfig().getString("messages.unban-success").replace("%target%", args[1])));
        return true;
    }

    private boolean handleUnmute(CommandSender sender, String[] args) {
        if (args.length < 2) {
            sender.sendMessage(plugin.getGuardPrefix() + plugin.color(plugin.getConfig().getString("messages.syntax").replace("%syntax%", "/pg unmute <joueur>")));
            return true;
        }
        sanctionService.unmute(args[1], sender.getName());
        sender.sendMessage(plugin.getGuardPrefix() + plugin.color(plugin.getConfig().getString("messages.unmute-success").replace("%target%", args[1])));
        return true;
    }

    private boolean handleHistory(CommandSender sender, String[] args) {
        if (args.length < 2) {
            sender.sendMessage(plugin.getGuardPrefix() + plugin.color(plugin.getConfig().getString("messages.syntax").replace("%syntax%", "/pg history <joueur>")));
            return true;
        }
        List<SanctionEntry> history = sanctionService.getHistory(args[1]);
        if (history.isEmpty()) {
            sender.sendMessage(plugin.getGuardPrefix() + plugin.color(plugin.getConfig().getString("messages.no-history").replace("%target%", args[1])));
            return true;
        }
        sender.sendMessage(plugin.getGuardPrefix() + plugin.color(plugin.getConfig().getString("messages.history-header").replace("%target%", args[1])));
    ...
    private void sendHelp(CommandSender sender) {
        sender.sendMessage(plugin.getGuardPrefix() + plugin.color("&fCommandes : &d/pg report, /pg panel, /pg ban, /pg tempban, /pg mute, /pg tempmute, /pg kick, /pg warn, /pg unban, /pg unmute, /pg history"));
    }

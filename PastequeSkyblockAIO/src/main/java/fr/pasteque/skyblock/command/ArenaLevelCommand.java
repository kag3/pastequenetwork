...
public class ArenaLevelCommand implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        player.sendMessage(plugin.color(plugin.prefix("arena") + plugin.getConfig().getString("messages.arena-level", "&fNiveau actuel : &d%level% &8| &fXP : &d%xp%/%next%")
                .replace("%level%", String.valueOf(data.getLevel()))
                .replace("%xp%", String.valueOf(data.getXp()))
                .replace("%next%", String.valueOf(next))));
        player.sendMessage(plugin.color(plugin.prefix("arena") + plugin.getConfig().getString("messages.arena-stats", "&fKills : &d%kills% &8| &fMorts : &d%deaths% &8| &fRatio : &d%ratio%")
